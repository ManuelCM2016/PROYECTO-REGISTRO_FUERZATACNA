...
function playBeep(ok: boolean) {
  try {
    ...
    osc.type = 'sine';
    osc.frequency.setValueAtTime(ok ? 880 : 330, ctx.currentTime);
    osc.frequency.exponentialRampToValueAtTime(ok ? 1320 : 220, ctx.currentTime + 0.2);
...
type ResultState = {
  type: 'success' | 'already_verified' | 'already_delivered' | 'not_found' | 'error';
  message: string;
  data?: any;
};
...
export default function VerificarPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleVerify = async (e?: FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/pollada/${id}/verificar`, {
        method: 'POST',
      ...
      if (data.success) {
        ...
        r = { type: 'already_verified', message: data.error, data: data.data };
        playBeep(true);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-primary-950 to-slate-950 p-4 flex flex-col">
      {/* Header */}
      <div className="text-center mb-6 pt-4">
        <div className="inline-flex items-center gap-2 bg-amber-500/20 border border-amber-500/40 rounded-full px-4 py-1.5 mb-3">
          <span className="text-amber-400 text-sm font-bold">🚪 CONTROL DE PUERTA</span>
        </div>
        {loadingPollada ? (
      ...
      <div className="max-w-md w-full mx-auto space-y-4 flex-1">
        ...
        <form onSubmit={handleVerify} className="space-y-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-4 rounded-2xl bg-surface-900/90 border-2 border-primary-800/60 text-[#f8f9f9] text-xl font-mono tracking-widest focus:outline-none focus:border-accent-500/80 placeholder:text-primary-600/50 placeholder:font-sans placeholder:text-base placeholder:tracking-normal"
            />
          </div>
          <button
            type="submit"
            disabled={dni.length < 8 || processing}
            className="w-full py-4 rounded-2xl text-base font-black tracking-wide transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed bg-gradient-to-r from-amber-500 to-amber-600 text-black hover:from-amber-400 hover:to-amber-500 shadow-lg shadow-amber-500/25 cursor-pointer"
          >
            {processing ? (
                    ...
                    <div className="mt-3 space-y-1 text-sm">
                      ...
                      {result.data.cantidad_tickets && (
                        <p className="text-amber-300 font-black text-lg">
                          🎟 {result.data.cantidad_tickets} ticket(s) → {result.data.cantidad_tickets} porción(es)
                        </p>
                      )}
                    </div>
                  )}
        ...
        {recentList.length > 0 && (
          <div className="bg-surface-900/60 border border-primary-900/40 rounded-2xl p-4">
            <p className="text-xs font-bold text-primary-300/60 mb-3">ÚLTIMOS VERIFICADOS</p>
