...
import { verificarTicket } from '@/lib/google-sheets';
...
export async function POST(
  ...
  try {
    ...
    const result = await verificarTicket({
      id_pollada: id,
      dni: cleanDni,
      verificado_por: session.username,
    });

    return NextResponse.json(result);
  } catch (error) {
    console.error('Error verificando ticket:', error);
    return NextResponse.json({ success: false, error: 'Error al verificar el ticket' }, { status: 500 });
  }
}
